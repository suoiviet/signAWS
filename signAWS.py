...
startdateYMD = datetime.datetime.strptime(startdate, '%m/%d/%Y').strftime('%Y%m%d')

if enddate == "":
  enddate = datetime.datetime.today().strftime('%m/%d/%Y')
enddateYMD = datetime.datetime.strptime(enddate, '%m/%d/%Y').strftime('%Y%m%d')

if debug == 1:
  print("currentdate="+currentdate)
  print("startdate="+startdate)
  print("enddate="+enddate)
  print("startdateYMD="+startdateYMD)
  print("enddateYMD="+enddateYMD)
  exit()

# ************* REQUEST VALUES *************
method = 'POST'
service = 'execute-api'
host = 'zzzyyyxxxuuu.execute-api.us-east-1.amazonaws.com'
region = 'us-east-1'
endpoint = 'https://zzzyyyxxxuuu.execute-api.us-east-1.amazonaws.com/prod/rest-endpoint'

# POST requests use a content type header.
# the content is JSON.
content_type = 'application/json'

# Request parameters in a JSON block.
request_parameters =  '{'
request_parameters +=  '"startdate": "'+startdate+'",'
request_parameters +=  '"enddate": "'+enddate+'"'
request_parameters +=  '}'
...
# Read AWS access key
access_key = 'AKIA12345678SOMEKEY'
secret_key = 'n+zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'
...
canonical_uri = '/prod/rest-endpoint'
...
# header, the headers must be included in the canonical_headers and signed_headers values, as noted earlier. Order here is not significant.
